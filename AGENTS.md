...
This is the `pooly/services/poold` Go module. It builds one binary:
...
Recent commits use short imperative subjects, for example `Embed timezone data`. Keep the first line focused on the behavior change; add body text only when context or tradeoffs matter.
